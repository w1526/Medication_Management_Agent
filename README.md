...
Plan 审批现在会先执行确定性的 M2 安全检查：`PASS`/`WARN` 才能激活，`BLOCK` 或安全引擎故障保持 `pending_confirmation`，不生成 occurrence，也不发送提醒。Scheduler 和 Outbox 发提醒前还会检查当前 Plan Version 的有效 Safety Check 与 `ruleset_version`。
...
因此默认 `PASS` 只表示当前结构检查范围内没有发现问题，不代表临床绝对安全。显式测试规则见 [data/safety_rules.example.json](data/safety_rules.example.json)，只使用 `TEST_DRUG_A/B/C` 虚构药物；项目没有接入真实 DDI、过敏、禁忌或 DrugBank/OpenFDA 数据库。
...
详见 [Phase 3 设计](docs/phase3_safety_check.md) 和 [Phase 3 实现报告](docs/phase3_safety_check_implementation_report.md)。
